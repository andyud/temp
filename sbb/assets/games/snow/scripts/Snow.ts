...
import { SnowReel } from './SnowReel';
import { Notice } from '../../../prefabs/popups/scripts/Notice';
import { SnowSpinBtn } from './SnowSpinBtn';
...
export class Snow extends Component {
    ...
    private readonly gameName = 'snowQueen';
    ...
    start() {
        ...
        this.btnSpin.getComponent(SnowSpinBtn).init((val) => {
            if (val == 0) {
            } else if (val == 1) {
                if (!this.isAutoSpin) {
                    this.spin(false, []);
        ...
        for (let i = 0; i < this.reels.length; i++) {
            arr = GameMgr.instance.shuffle(arr);
            this.reels[i].getComponent(SnowReel).init(arr, this.items);
            this.reels[i].getComponent(SnowReel).setReelCallback(this.endSpin.bind(this));
        }
        //--bonus
        for (let i = 0; i < this.arrPlayBonusItem.length; i++) {
            this.arrPlayBonusItem[i].getComponent(SnowBonusItem).init((idx: number) => {
                ...
                if (this.countBonusRemain == 0) { //--disable touch
                    for (let k = 0; k < this.arrPlayBonusItem.length; k++) {
                        this.arrPlayBonusItem[k].getComponent(SnowBonusItem).setTouchEnable(false);
                    }
                }

                if (this.countBonusRemain > 0) {
                    for (let k = 0; k < this.arrPlayBonusItem.length; k++) {
                        this.arrPlayBonusItem[k].getComponent(SnowBonusItem).setTouchEnable(true);
                    }
                    ...
                    const timeout1 = setTimeout(() => {
                        ...
                        for (let ii = 0; ii < this.arrPlayBonusItem.length; ii++) {
                            ...
                            this.arrPlayBonusItem[ii].getComponent(SnowBonusItem).setValue(currVal2,'','#ffffff');
                        }
            ...
            case "loginRes":
                ...
                if (this.loginRes && this.loginRes.reelInfo && this.loginRes.reelInfo.normal && this.loginRes.reelInfo.normal.length > 0) {
                    ...
                    for (let i = 0; i < arr.length; i++) {
                        ...
                        this.reels[i].getComponent(SnowReel).init(GameMgr.instance.shuffle(arrTemp), this.items);
                    }
                }
                break;
            case "spinRes":
                ...
                let reelsRunTime = [];
                for (let i = 0; i < this.reels.length; i++) {
                    let speed = this.reels[i].children.length * this.snowConfig.reelsSpeed;
                ...
                if (this.spinRes && this.spinRes.lineKeys && this.spinRes.lineKeys.length > 0) {
                    for (let i = 0; i < this.spinRes.lineKeys.length; i++) {
                        ...
                        for (let j = 0; j < arr.length; j++) {
                            ...
                            this.reels[i].children[j].getComponent(SnowItem).setTexture(tex, texId);
                        }
                    }
                }
                Promise.all(self.reels.map(function (reel, index) {
                    reel.getComponent(SnowReel).spin(index, reelsRunTime[index]);//1s
                }))
                break;
            case "spinDebug":
                this.spinRes = data;
    ...
    onClick(button: Button) {
        ...
        switch (button.node.name) {
            ...
            case 'btnBonusInfo':
                ...
                for (let i = 0; i < this.arrPlayBonusItem.length; i++) {
                    this.arrPlayBonusItem[i].getComponent(SnowBonusItem).reset();
                }
                if (this.isFreeSpin) {
    ...
    preSpin() {
        ...
        if (this.isAutoSpin) {
            ...
            const timeOut4 = setTimeout(() => {
                ...
                // this.jackpotNode.active = false;
                this.spin(false, []);
            }, this.spinRes.totalWinBalance > 0 ? this.snowConfig.showResultDelay : 0)
        }
    }
    spin(isDebug: boolean = false, data: any) {
        ...
        for (let i = 0; i < this.reels.length; i++) {
            for (let j = 0; j < this.reels[i].children.length; j++) {
                this.reels[i].children[j].getComponent(SnowItem).stopZoomAnim();
            }
        }
        this.lineEffects = [];
            ...
            for (let kk = 0; kk < coord.length; kk++) {
                ...
                for (let ii = 0; ii < this.serverMatrix.length; ii++) {
                    ...
                    for (let jj = 0; jj < row.length; jj++) {
                        ...
                        let val2 = row[jj];
                        if (val1 == val2) {
                            this.reels[jj].children[startCol+ii].getComponent(SnowItem).zoomAnim();
                            //console.log(`anim [${jj},${2 - ii} val: ${val1}]`)
                            let texId = this.reels[jj].children[startCol+ii].getComponent(SnowItem).idx;
                            if (texId == this.ICON_MAPPING.wild) {//wild
                                this.reels[jj].children[startCol+ii].getComponent(SnowItem).runWild(this.items);
                            }
                        }
                    }
                }
        ...
        if (this.spinRes.totalWinBalance > 0) {
            ...
            let timeout10 = setTimeout(() => {
                ...
                for (let i = 0; i < ef; i++) {
                    ...
                    coint.getComponent(SnowCoinEff).init(startPos, endPos);
                }
